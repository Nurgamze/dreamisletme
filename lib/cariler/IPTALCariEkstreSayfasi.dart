...
class CariEkstre extends StatefulWidget {
  ...
  final Cari data;
  CariEkstre(this.ekstreTarihi,{this.ekstreSonTarihi,required this.data});
  @override
  _CariEkstreState createState() => _CariEkstreState();
}
...
class _CariEkstreState extends State<CariEkstre> {

  var sipraisJson;
  bool loading = false;
  String borc = "0",alacak = "0",bakiye = "0";
  DataGridController _dataGridController = DataGridController();
  late CariEkstreDataSource _cariEkstreDataSource;

  List<CariEkstreGridModel> aramaList = [];
  ...
  void initState() {
    ...
    _cariEkstreDataSource = CariEkstreDataSource(_dataGridController);
    _ekstreGetir();
    super.initState();
  ...
  _ekstreGetir() async {
    var response = await http.get(Uri.parse("${Sabitler.url}/api/CariHesapEkstresi?VtIsim=${UserInfo.activeDB}&Customer=false&cariKod=${widget.data.kod}&ekstreTarihi=${widget.ekstreTarihi.month}.${widget.ekstreTarihi.day}.${widget.ekstreTarihi.year}&ekstreSonTarihi=${widget.ekstreSonTarihi}&ozet=false&Mobile=true&DevInfo=${TelefonBilgiler.userDeviceInfo}&AppVer=${TelefonBilgiler.userAppVersion}&UserId=${UserInfo.activeUserId}"),headers: {"apiKey" : Sabitler.apiKey});
    if(response.statusCode == 200) {
      setState(() {
        var ekstreDetay = json.decode(response.body);
        for(var ekstre in ekstreDetay){
          CariEkstreGridModel cariEkstreler = CariEkstreGridModel(ekstre["bakiye"],ekstre["belgeTarihi"] != null ? DateTime.parse(ekstre["belgeTarihi"]) : null, ekstre["cinsi"], ekstre["evrakSeri"], ekstre["evrakSira"],ekstre["evrakTipi"],ekstre["isTarihi"] != null ? DateTime.parse(ekstre["isTarihi"]) : null, ekstre["kayit"],ekstre["meblag"], ekstre["normalIade"], ekstre["tip"],ekstre["vadeTarihi"] != null ? DateTime.parse(ekstre["vadeTarihi"]) : null);
          cariEkstreGridList.add(cariEkstreler);
        }

        alacak = cariEkstreGridList[cariEkstreGridList.length-3].cinsi;
        borc = cariEkstreGridList[cariEkstreGridList.length-2].cinsi;
        bakiye = cariEkstreGridList[cariEkstreGridList.length-1].cinsi;
        cariEkstreGridList.removeRange(cariEkstreGridList.length-4, cariEkstreGridList.length);

        evrakTipiFiltreList.clear();
        tipFiltreList.clear();
        cinsiFiltreList.clear();
        for(var cariEkstre in cariEkstreGridList){
          bool addC = true;
          bool addS = true;
          bool addT = true;
  ...
  PdfGrid getBABGrid() {
    ...
    if(cariEkstreGridList[0].tip == "Alacak"){
      devirMeblag = cariEkstreGridList[0].bakiye! + cariEkstreGridList[0].meblag!;
    }else if(cariEkstreGridList[0].tip == "Borç") {
      devirMeblag = cariEkstreGridList[0].bakiye! - cariEkstreGridList[0].meblag!;
    }
    double toplamBorc = devirMeblag;
    double toplamAlacak = 0.0;
    for(int i = 0; i < cariEkstreGridList.length; i++){

      if(cariEkstreGridList[i].tip == "Alacak") toplamAlacak+= cariEkstreGridList[i].meblag!;
      if(cariEkstreGridList[i].tip == "Borç") toplamBorc+= cariEkstreGridList[i].meblag!;
    }
  ...
  PdfGrid getGrid() {
    ...
    for(int i = 0; i < cariEkstreGridList.length; i++){
      var formatMeblag= Foksiyonlar.formatMoney(cariEkstreGridList[i].meblag);
      var formatbakiye= Foksiyonlar.formatMoney(cariEkstreGridList[i].bakiye);
      var formatBelgeTarihi = DateFormat('dd.MM.yyyy').format(cariEkstreGridList[i].belgeTarihi!);
      var formatVadeTarihi = DateFormat('dd.MM.yyyy').format(cariEkstreGridList[i].vadeTarihi!);
      if(i == 0 ){
        if(cariEkstreGridList[i].tip == "Alacak"){
          double devirBakiye = cariEkstreGridList[0].bakiye! + cariEkstreGridList[0].meblag!;
      ...
      addProducts(cariEkstreGridList[i].tip, cariEkstreGridList[i].evrakTipi, "${cariEkstreGridList[i].evrakSeri}-${cariEkstreGridList[i].evrakSira}", formatBelgeTarihi, formatMeblag, formatbakiye, formatVadeTarihi, grid);
    }
    addProducts("","","","..", "","",".." ,grid);
    addProducts("","","","..", "","",".." ,grid);
  ...
  _gridAra(List<String?> a, List<String?> b, List<String?> c,) {
    List<CariEkstreGridModel> arananlarList = [];
    ...
    setState(() {
      ...
      cariEkstreGridList = arananlarList;
      if(arananlarList.isNotEmpty && (a.isNotEmpty || b.isNotEmpty || c.isNotEmpty || _selectedTime != null)){
        cariEkstreGridList.add(CariEkstreGridModel(0, null, "", "", "", "", null, "", toplamMeblag, "", "Toplam", null));
      }
...
class CariEkstreDataSource extends DataGridSource {
  ...
  CariEkstreDataSource(this.dataGridController) {
    buildDataGridRows();
  }
  void buildDataGridRows() {
    dataGridRows = cariEkstreGridList.map<DataGridRow>((e) => DataGridRow(
        cells: [
