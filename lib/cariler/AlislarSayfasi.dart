...
class AlislarSayfasi extends StatefulWidget {
  ...
  AlislarSayfasi({required this.data});
  @override
  _AlislarSayfasiState createState() => _AlislarSayfasiState();
}
...
class _AlislarSayfasiState extends State<AlislarSayfasi> {
  ...
  late AlislarDataSource _alislarDataSource;

  bool loading = false;
  List<AlislarGridModel> aramaList = [];
  ...
  void initState() {
    ...
    super.initState();
    _alislarDataSource = AlislarDataSource(_dataGridController);
    _alislariGetir();
              ...
              children: [
                ...
                Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        Text("NELER ALINMIŞ",style: GoogleFonts.roboto(textStyle: TextStyle(fontSize: 14,color: Colors.white,fontWeight: FontWeight.bold)),textAlign: TextAlign.center,),
    ...
    return SfDataGridTheme(
      ...
      child: SfDataGrid(
        ...
        columns: <GridColumn> [
          ...
          dreamColumn(columnName: 'turu', label: 'TÜR',alignment: Alignment.centerLeft),
          dreamColumn(columnName: 'evrak', label: 'EVRAK',alignment: Alignment.centerLeft,),
        ],
  ...
  _satisAra() async {
    List<AlislarGridModel> arananlarList = [];
    ...
    setState(() {
      alislarGridList = arananlarList;
      _alislarDataSource = AlislarDataSource(_dataGridController);
    });
  }

  _alislariGetir() async {
    alislarGridList.clear();
    var response = await http.get(Uri.parse("${Sabitler.url}/api/AlisSatislar?VtIsim=${UserInfo.activeDB}&CariKodu=${widget.data.kod}&Alislar=true&Mobile=true&DevInfo=${TelefonBilgiler.userDeviceInfo}&AppVer=${TelefonBilgiler.userAppVersion}&UserId=${UserInfo.activeUserId}"),headers: {"apiKey" : Sabitler.apiKey});
    if(response.statusCode == 200){
      var alislarJson = jsonDecode(response.body);
      for(var alislar in alislarJson) {
        AlislarGridModel alis = AlislarGridModel(alislar['stokKodu'], alislar['stokAdi'].toString(), DateTime.parse(alislar['tarih'].toString()), alislar['miktar'],alislar['birim'],alislar['birimFiyat'],
            alislar['paraBirimi'],alislar['dovizBirimFiyat'],alislar['doviz'],alislar['kur'],alislar['turu'],alislar['evrak']);
          alislarGridList.add(alis);

      }
      setState(() {
        _alislarDataSource = AlislarDataSource(_dataGridController);
        loading = true;
        aramaList = alislarGridList;
      });
...
class AlislarDataSource extends DataGridSource {
  ...
  AlislarDataSource(this.dataGridController) {
    buildDataGridRows();
  }
  void buildDataGridRows() {
    dataGridRows = alislarGridList.map<DataGridRow>((e) => DataGridRow(
        cells: [
