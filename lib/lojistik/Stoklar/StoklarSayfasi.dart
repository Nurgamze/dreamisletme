...
class _StoklarSayfasiState extends State<StoklarSayfasi> {
  ...
  _stokAutoComplateGetir() async {
    ...
    print("Stok auto complete içindeyim");
    print("VTisim: ${UserInfo.activeDB}");
    print("FullAccess: ${UserInfo.fullAccess}");
    print("Mobile: true");
    print("DevInfo: ${TelefonBilgiler.userDeviceInfo}");
    print("AppVer: ${TelefonBilgiler.userAppVersion}");
    print("UserId: ${TelefonBilgiler.userAppVersion}");
    print("AppVer: ${UserInfo.activeUserId}");



    var response = await http.get(Uri.parse("${Sabitler.url}"
        "/api/StokAutoComplate?VtIsim=MikroDB_V16_12&FullAccess=${UserInfo.fullAccess}&Mobile=true&DevInfo=${TelefonBilgiler
  ...
  _stoklariGetir(bool isScan) async {
    ...
    print("VtIsim : MikroDB_V16_12");
    print(UserInfo.aktifSubeNo);
    stoklarGridList.clear();
    var body = jsonEncode({
      "VtIsim" : "MikroDB_V16_12",
  ...
  Future<void> _filtreGetir() async {
    var response = await http.get(Uri.parse("${Sabitler.url}/api/StokFiltre?vtIsim=MikroDB_V16_12"),headers: {"apiKey" : Sabitler.apiKey});
    if(response.statusCode == 200){
      filtreList.clear();
    ...
    response = await http.get(Uri.parse("${Sabitler.url}/api/StokFiltreGetir?vtIsim=MikroDB_V16_12"),headers: {"apiKey" : Sabitler.apiKey});
    if(response.statusCode == 200){
      var filtreler = jsonDecode(response.body);
      for(var filtre in filtreler){
        switch (filtre["Grup"])
